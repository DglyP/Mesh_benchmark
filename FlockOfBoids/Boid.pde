class Boid {
  ...
  void render() {
    ...
    vertex(-3 * sc, -2 * sc, 0);
    println("1: " + (3 * sc) + ", " + (0) + ", " + (0));
    println("2: " + (-3 * sc) + ", " + (2 * sc) + ", " + (0));
    println("3: " + (-3 * sc) + ", " + (-2 * sc) + ", " + (0));
    println("4: " + (-3 * sc) + ", " + (0) + ", " + ( 2 * sc ));
    endShape();*/
    ArrayList<Vector> vectorSend = new ArrayList();
    vectorSend.add(new Vector(3 * sc, 0, 0));
    vectorSend.add(new Vector(-3 * sc, 2 * sc, 0));
    vectorSend.add(new Vector(-3 * sc, -2 * sc, 0));
    vectorSend.add(new Vector(-3 * sc, 0, 2 * sc));
    if(meshVV){
      RepresentationV_V fig = new RepresentationV_V(vectorSend, kind);
      if(!retained)
        fig.structInmediateShape();
      else
        shape(fig.structRetainedShape());
      }
    else{
      RepresentationF_V fig = new RepresentationF_V(vectorSend, kind);
      if(retained)
        shape(fig.structRetainedShape());
      else
        fig.structInmediateShape();
